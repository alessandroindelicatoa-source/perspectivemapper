# Rebuild full app_v2.py with complete code (not placeholder)
import os, zipfile, textwrap
from PIL import Image, ImageDraw

base = "/mnt/data/PerspectiveMapper_v2_full"
assets = os.path.join(base, "assets")
streamlit_dir = os.path.join(base, ".streamlit")
os.makedirs(assets, exist_ok=True)
os.makedirs(streamlit_dir, exist_ok=True)

app_v2_code = textwrap.dedent("""
    import os, re, json
    ...
    try:
        ...
        bertopic_model=BERTopic(embedding_model=embedder)
    ...
    if st.button("Generate PDF"):
        ...
        story=[Paragraph("PerspectiveMapper v2 Report", styles["Title"])]
        doc.build(story)
        with open("results.pdf","rb") as f:
            st.download_button("Download PDF",data=f,file_name="results.pdf")
""")

with open(os.path.join(base,"app_perspectivemapper.py"),"w") as f: f.write(app_v2_code)
...
reportlab
"""
with open(os.path.join(base,"requirements.txt"),"w") as f: f.write(reqs)

with open(os.path.join(base,"README.md"),"w") as f: f.write("# PerspectiveMapper v2 Full App")

img=Image.new("RGB",(800,400),(245,248,255))
draw=ImageDraw.Draw(img)
draw.text((40,140),"PerspectiveMapper v2",fill=(20,40,80))
img.save(os.path.join(assets,"logo.png"))

with open(os.path.join(streamlit_dir,"secrets_template.toml"),"w") as f: f.write("[passwords]\\nuser='your_password'\\n")

zip_path="/mnt/data/PerspectiveMapper_v2_full.zip"
with zipfile.ZipFile(zip_path,"w",zipfile.ZIP_DEFLATED) as z:
    for root,dirs,files in os.walk(base):
        for file in files:
            full=os.path.join(root,file)
            arc=os.path.relpath(full,base)
            z.write(full,os.path.join("PerspectiveMapper_v2_full",arc))
